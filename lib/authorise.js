...
function getBearerToken (done) {
  var headerToken = this.req.headers.authorization,
      ...
      postToken = this.req.payload ? this.req.payload.access_token : undefined,
      sessionToken = this.req.session ? this.req.session.get('access_token') : undefined;
  ...
  if (methodsUsed > 1) {
    return done(error('invalid_request',
      'Only one method may be used to authenticate at a time (Auth header,  ' +
        'GET or POST).'));
  } else if (methodsUsed === 0 && !sessionToken) {
  ...
  if (postToken) {
    ...
    if (this.req.mime !== 'application/x-www-form-urlencoded') {
  ...
  this.bearerToken = headerToken || postToken || getToken || sessionToken;
  done();
}
...
function checkToken (done) {
  ...
  this.model.getAccessToken(this.bearerToken, function (err, token) {
    ...
    // Expose params
    self.req.app.oauth = { bearerToken: token };
    self.req.app.user = token.user ? token.user : { id: token.userId };
